...
def refresh_history():
    for item in tree.get_children():
        tree.delete(item)
    for pwd in history[::-1]:  # Отображаем в обратном порядке (последние сверху)
...
ttk.Checkbutton(frame_options, text="Цифры", variable=var_digits).grid(row=1, column=0, padx=5, pady=2, sticky="w")
ttk.Checkbutton(frame_options, text="Буквы", variable=var_letters).grid(row=2, column=0, padx=5, pady=2, sticky="w")
ttk.Checkbutton(frame_options, text="Спецсимволы", variable=var_special).grid(row=3, column=0, padx=5, pady=2, sticky="w")
...
refresh_history()  # Обновляем таблицу при запуске
